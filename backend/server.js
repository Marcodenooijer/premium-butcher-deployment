...
(async () => {
  try {
    ...
    console.log('✅ Google WalletGenericy class initialized');
  } catch (error) {
...
app.get('/api/google-wallet/pass', authenticateUser, async (req, res) => {
  try {
    // Get customer data
    const customerResult = await pool.query(
      `SELECT id, name, email, ean13_barcode, member_since
       FROM customers
    ...
    // Create/update Google Wallet pass (Generic Pass - No Loyalty Points)
    const saveUrl = await googleWallet.createGenericObject({
      id: customer.id,
      name: customer.name,
